...
export default function NotificationHistory({ onClose }: NotificationHistoryProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl rounded-3xl border border-white/10 bg-[#17181a] shadow-2xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between border-b border-white/5 px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded-xl border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-medium text-white/70 hover:bg-white/10 hover:text-white transition-all flex items-center gap-2"
            >
              <Trash2 size={14} />
              Effacer tout
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="mt-1 text-xs text-white/30">Les notifications récentes apparaîtront ici</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        <span className="text-[10px] text-white/40">{formatTime(notification.timestamp)}</span>
                        {notification.action && (
                          <button
                            onClick={notification.action.onClick}
                            className="rounded-lg border border-white/10 bg-white/5 px-2 py-1 text-[10px] font-medium text-white/70 hover:bg-white/10 hover:text-white transition-all"
                          >
                    ...
                    <button
                      ...
                      className="rounded-lg p-1 text-white/30 hover:bg-white/10 hover:text-red-400 transition-all"
                      title="Supprimer"
                    >
                      <X size={12} />
                    </button>
